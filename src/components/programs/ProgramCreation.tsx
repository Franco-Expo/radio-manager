...
type ProgramCreationProps = {
  ...
  onTakeUpdate: (takeId: string, songs: {
    id: string;
    title: string;
    news: string;
...
export function ProgramCreation({
  ...
  const selectedProgram = selectedProgramId ? programs.find(p => p.id === selectedProgramId) : undefined;
  ...
  return <div className="flex flex-col h-full">
      <div className="flex flex-col justify-center items-center my-8">
        {!selectedProgram && <p className="text-center text-lg mb-4 max-w-md font-bold">
      ...
      {selectedProgram && takes.length > 0 && <div className="flex-1 p-4">
          <ProgramHeader programName={selectedProgram.name} />
          <TakeManager takes={takes} selectedProgram={selectedProgram} onTakeCreate={onTakeCreate} onTakeUpdate={onTakeUpdate} onTakeDelete={onTakeDelete} onSaveProgram={onProgramSave} onSaveComplete={handleReturnToNewProgram} />
        </div>}
    </div>;
}